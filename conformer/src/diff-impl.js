...
const refResult = run(config.reference);
const implResult = run(impl);

if (refResult.error) {
  process.stderr.write(`reference (${config.reference.name}): ${refResult.error}\n`);
}
if (implResult.error) {
...
const refFile = path.join(tmpDir, `${config.reference.name}.json`);
