...
const SCRIPT = path.join(__dirname, 'index.rb');
let tmpDir;

beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'harness-graphql-ruby-test-'));
});
...
function run(schemaPath, queryPath, variablesPath) {
  const args = ['exec', 'ruby', SCRIPT, schemaPath, queryPath];
  if (variablesPath) args.push(variablesPath);
  const stdout = execFileSync('bundle', args, {
    cwd: __dirname,
    encoding: 'utf8',
    timeout: 30_000,
  });
...
describe('graphql-ruby conformer-harness', () => {
